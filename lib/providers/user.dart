...
class ProfileProvider extends ChangeNotifier {
  ...
  Future<void> _onAuthStateChanged(FirebaseUser firebaseUser) async {
    ...
    if (_user != null) {
      ...
      if (!_user.isAnonymous) {
        ...
        final String profilePixLocation =
            '${DBLocations.profilePix + _user.uid}.jpg';
        final String profilePixThumbLocation =
            '${DBLocations.profilePixThumb + _user.uid}.jpg';

        if (_awaitingProfilePix != null) {
          //update saved info if user signedup recently
          final File compressed = await ManageImage.resizeImage(
              image: _awaitingProfilePix, quality: 30);
          setProfile();
          ...
          await _uploadDoc(profilePixThumbLocation, compressed).then((link) {
          ...
          streamSub =
              reference.document(_user.uid).snapshots().listen((snapshot) {
            //print(snapshot.data);
    ...
    await _auth
        .verifyPhoneNumber(
            ...
            timeout: const Duration(
                seconds: 120), //time before another code can be sent
  ...
  void uploadCoverPhoto(File image) async {
    ...
    _uploadTask =
        cloudStorageService.startUpload(file: image, filePath: fileInfo);
    notifyListeners();
  ...
  void uploadProfilePix(File fullImage, File compressedImage) async {
    ...
    CloudStorageService cloudStorageService = CloudStorageService();
    _uploadTask =
        cloudStorageService.startUpload(file: fullImage, filePath: fileInfo);
    notifyListeners();
    ...
    await (await _uploadTask.onComplete)
      ...
      notifyListeners();
      _uploadTask = cloudStorageService.startUpload(
          file: compressedImage, filePath: thumbinfo);
      notifyListeners();
  ...
  void setProfile() {
    _userProfile.userId = user.uid; //store user id for future use.
    CollectionReference reference = _firestore.collection('users');
    reference = _firestore.collection('users');
    reference.document(_user.uid).setData(_userProfile.toMap);
  }

  set awaitigProfilePix(File pix) {
    // keep profile pix awaiting user signin
